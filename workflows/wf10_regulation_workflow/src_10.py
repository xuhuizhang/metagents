...
model = LLMList("glm4_p").chat_model(0.3, streaming=True)
# model = LLMList("deepseek_chat").chat_model(1.0, streaming=True)
...
def es_rag(question):
    ...
    retriever_docs = requests.post(
        'http://10.26.9.148:31817/vector_stores/60/search',
...
class CreateRegulationInput(BaseModel):
    question: str = Field(..., description="用户提出的金融监管规定或监管政策方面的问题")


class RegulationSearch(BaseTool):
    """
    这是一个金融监管规定或监管政策的增强检索（RAG）的工具。
    本工具结合用户金融监管规定或监管政策问题，从而进行RAG的使用。
    """
    name: str = "RegulationSearch"
    description: str = """
    这是一个金融监管规定或监管政策的增强检索（RAG）的工具。
    本工具结合用户金融监管规定或监管政策问题，从而进行RAG的使用。
    """
    args_schema: Type[BaseModel] = CreateRegulationInput
...
def call_llm(state: MessagesState) -> MessagesState:
    ...
    """
    messages = state["messages"]
    llm_with_tools = model.bind_tools([RegulationSearch()], tool_choice="auto")
    # prompt = ChatPromptTemplate.from_messages(
    #     [
    #         (
    #             'system',
    #             '''
    #             如果发现有从工具中收到消息，请使用工具消息作答，不要编造答案。
    #             '''
    #         ), MessagesPlaceholder(variable_name='message')
    #     ]
    # )
    # llm = prompt | llm_with_tools
    # response = llm.invoke({"message": messages})
    response = llm_with_tools.invoke(messages)
    return {"messages": [response]}  # type: ignore


def regulation_build():
    tool_node = ToolNode([RegulationSearch()])
...
def regulation_workflow():
    _graph = regulation_build()
    messages = []
    stop = True
    while stop:
        print("*" * 20)
